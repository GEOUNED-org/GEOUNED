...
def buildCAD(UnivCell,data,config):
    ...
    UniverseCut = True 
    if 'Ustart'   not in config.keys(): config['Ustart'] = 0
    if 'levelMax' not in config.keys(): config['levelMax'] = 'all'
    UnivCell.name = 0
...
def interferencia(container,cell,mode='slice'):
    ...
    cellParts = []
    for s in solids:
        if container.shape.isInside(s.CenterOfMass,0.0,False) : cellParts.append(s)
...
def BuildUniverse(startInfo,ContainerCell,AllUniverses,universeCut=True,duplicate=False):
    ...
    print('Build Universe {} in container cell {}'.format(ContainerCell.FILL,ContainerCell.name))       
    fails = []
    for NTcell in Universe.values():
        ...
        if universeCut and ContainerCell.shape : cell.shape = interferencia(ContainerCell,cell)
