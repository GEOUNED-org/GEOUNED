...
def get_id(facein, surfaces, tolerance):

    surfin = str(facein)
    if surfin == "<Plane object>":
        if is_parallel(facein.Axis, FreeCAD.Vector(1, 0, 0), tolerance.pln_angle):
            p = "PX"
        elif is_parallel(facein.Axis, FreeCAD.Vector(0, 1, 0), tolerance.pln_angle):
            p = "PY"
        elif is_parallel(facein.Axis, FreeCAD.Vector(0, 0, 1), tolerance.pln_angle):
            p = "PZ"
        else:
            p = "P"
        ...
        for s in surfaces[p]:
            if BF.is_same_plane(
                facein,
                s.Surf,
                dtol=tolerance.pln_distance,
                atol=tolerance.pln_angle,
                rel_tol=tolerance.relativeTol,
            ):
                return s.Index
        ...
        for s in surfaces["Cyl"]:
            if BF.is_same_cylinder(
                facein,
                s.Surf,
                dtol=tolerance.cyl_distance,
                atol=tolerance.cyl_angle,
                rel_tol=tolerance.relativeTol,
            ):
                return s.Index
        ...
        for s in surfaces["Cone"]:
            if BF.is_same_cone(
                facein,
                s.Surf,
                dtol=tolerance.kne_distance,
                atol=tolerance.kne_angle,
                rel_tol=tolerance.relativeTol,
            ):
                return s.Index
        ...
        for s in surfaces["Sph"]:
            if BF.is_same_sphere(
                facein, s.Surf, tolerance.sph_distance, rel_tol=tolerance.relativeTol
            ):
                return s.Index
        ...
        for s in surfaces["Tor"]:
            if BF.is_same_torus(
                facein,
                s.Surf,
                dtol=tolerance.tor_distance,
                atol=tolerance.tor_angle,
                rel_tol=tolerance.relativeTol,
            ):
                return s.Index
...
def gen_plane(face, solid, tolerance):
    ...
    if str(surf) == "<Cylinder object>":
        return gen_plane_cylinder(face, solid, tolerance)
    if str(surf) == "<Cone object>":
        return gen_plane_cone(face, solid, tolerance)
    if str(surf) == "Sphere":
        return gen_plane_sphere(face, solid, tolerance)
...
def gen_plane_cylinder(face, solid, tolerance):
    ...
    for i, face2 in enumerate(solid.Faces):
        if face2.Area < tolerance.min_area:
            if opt.verbose:
                print(
                    f"Warning: {str(surf)} surface removed from cell definition. Face area < Min area ({face2.Area} < {tolerance.min_area}) "
                )
            continue
...
def gen_plane_cylinder_old(face, solid, tolerance):
...
def gen_plane_cone(face, solid, tolerance):
...
def gen_plane_cone_old(face, solid, tolerance):
...
def gen_torus_annex_u_planes(face, u_params, tolerance):

    if is_parallel(face.Surface.Axis, FreeCAD.Vector(1, 0, 0), tolerance.tor_angle):
        axis = FreeCAD.Vector(1, 0, 0)
    elif is_parallel(face.Surface.Axis, FreeCAD.Vector(0, 1, 0), tolerance.tor_angle):
        axis = FreeCAD.Vector(0, 1, 0)
    elif is_parallel(face.Surface.Axis, FreeCAD.Vector(0, 0, 1), tolerance.tor_angle):
    ...
    if is_same_value(abs(u_params[1] - u_params[0]), math.pi, tolerance.value):
        d = axis.cross(p2 - p1)
        d.normalize()
...
def gen_torus_annex_u_planes_org(face, u_params, tolerance):
...
def gen_torus_annex_v_surface(face, v_params, tolerance, force_cylinder=False):
    if is_parallel(face.Surface.Axis, FreeCAD.Vector(1, 0, 0), tolerance.tor_angle):
        axis = FreeCAD.Vector(1, 0, 0)
    elif is_parallel(face.Surface.Axis, FreeCAD.Vector(0, 1, 0), tolerance.tor_angle):
        axis = FreeCAD.Vector(0, 1, 0)
    ...
    if is_same_value(z1, z2, tolerance.distance):
        surf_type = "Plane"
    ...
    elif is_same_value(d1, d2, tolerance.distance) or force_cylinder:
        surf_type = "Cylinder"
        radius = min(d1, d2)
        center = face.Surface.Center
        if is_same_value(d1, face.Surface.MajorRadius, tolerance.distance):
            v_mid = (v_params[0] + v_params[1]) * 0.5
...
def cellDef(meta_obj, surfaces, universe_box, tolerance):
    ...
    for isol, solid in enumerate(solids):
        ...
        for iface, face in enumerate(solid_gu.Faces):
            ...
            if abs(face.Area) < tolerance.min_area:
                if opt.verbose:
                    print(
                        f"Warning: {surface_type} surface removed from cell definition. Face area < Min area ({face.Area} < {tolerance.min_area}) "
                    )
                continue
            if face.Area < 0:
                if opt.verbose:
            ...
            if (
                ...
                # cone additional plane is added afterward
                id_face = get_id(face.Surface, surfaces, tolerance)
                ...
                try:
                    plane = gen_plane(face, solid_gu, tolerance)
                ...
                if plane is not None:
                    ...
                    if exist:
                        ...
                        if is_opposite(p.Surf.Axis, pp.Surf.Axis, tolerance.angle):
                            id = -id
                    id *= sign
                ...
                if (
                    is_parallel(
                        face.Surface.Axis, FreeCAD.Vector(1, 0, 0), tolerance.angle
                    )
                    or is_parallel(
                        face.Surface.Axis, FreeCAD.Vector(0, 1, 0), tolerance.angle
                    )
                    or is_parallel(
                        face.Surface.Axis, FreeCAD.Vector(0, 0, 1), tolerance.angle
                    )
                ):

                    idT = get_id(face.Surface, surfaces, tolerance)
                    ...
                    if not u_closed:
                        ...
                        if exist:
                            ...
                            if is_opposite(
                                plane.Surf.Axis, p.Surf.Axis, tolerance.pln_angle
                            ):
                                id1 = -id1
                    ...
                    if orient == "Forward":
                        ...
                        if VClosed:
                            ...
                            surf_params, surf_type, in_surf = gen_torus_annex_v_surface(
                                face, VminMax, tolerance, opt.forceCylinder
                            )
                ...
                id = get_id(face.Surface, surfaces, tolerance)
                if surface_type == "<Cone object>":
                    cones.add(-id)
                ...
                if surface_type == "<Plane object>":
                    ...
                    if is_opposite(face.Surface.Axis, s.Surf.Axis, tolerance.pln_angle):
                        var = -var
...
def extra_plane_cyl_face(face, box, surfaces, tolerance):
    wire = face.OuterWire
    planes_id = []
    for e in wire.OrderedEdges:
        ...
        if curve[0:6] == "Circle" or curve == "<Ellipse object>":
            ...
            if exist:
                pp = surfaces.get_surface(id)
                if is_opposite(plane.Surf.Axis, pp.Surf.Axis, tolerance.pln_angle):
                    id = -id
            planes_id.append(id)
    return planes_id


def add_cone_plane(definition, cones_list, surfaces, universe_box, tolerance):
    ...
    for cid in cones_list:
        ...
        if (
            is_parallel(cone.Surf.Axis, x_axis, tolerance.angle)
            or is_parallel(cone.Surf.Axis, y_axis, tolerance.angle)
            or is_parallel(cone.Surf.Axis, z_axis, tolerance.angle)
        ):
            continue
        ...
        if exist:
            ...
            if is_opposite(plane.Surf.Axis, p.Surf.Axis, tolerance.pln_angle):
                pid = -pid
