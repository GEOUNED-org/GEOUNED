...
def complementary(ccell,outter=True) :
    ...
    wrkcell.str=re.sub(interblk,r"\g<previous>&\g<next>",wrkcell.str)  # change intersection separate by blank space ie: "number number"
    wrkcell.str=re.sub(interblk,r"\g<previous>&\g<next>",wrkcell.str)  # 2nd pass intersection blank space (require 2 pass)
    wrkcell.str=re.sub(intercls,r"\g<previous>&\g<next>",wrkcell.str)  # change intersection close parenthesis ie: ") number"
    wrkcell.str=re.sub(interopn,r"\g<previous>&\g<next>",wrkcell.str)  # change intersection open  parenthesis ie: "number ("
    wrkcell.str=re.sub(intercop,r"\g<previous>&\g<next>",wrkcell.str)  # change intersection close-open  parenthesis ie: ") ("
...
kept in the line if 'full' option is False (default)"""
   ...
   def SplitCell(self):
    ...
    # insert '&' as intersection operator
       geom=re.sub(interblk,r"\g<previous>&\g<next>",geom)  # change intersection separate by blank space ie: "number number"
...
def remove_hash(cards,cname,keepComments=True):
   def remove(card,cname,keepComments):
       ...
       if (not cardstr.hproc) or (cardstr.stat['hash'] == 0) :return cardstr.geom  # no complementary operator or cannot be # cannot be removed
