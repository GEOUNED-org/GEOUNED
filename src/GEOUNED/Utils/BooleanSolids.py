...
class CTelement:
   ...
   def getTranspose(self):
      if self.diagonal : return self.val
      return  CTelement((self.val[0],self.val[3],self.val[2],self.val[1]),self.S2,self.S1) 

   def getDependence(self):
      if self.diagonal : 
         if   self.val ==  0  : return True,False
         elif self.val == -1  : return 'Null',False
         else                 : return True,'Null'

      Ones = sum(self.val)
      if Ones == 4   : return None,None
      elif Ones == 3  :
         ind = self.val.index(0)
         if   ind == 0 : return False,None
         elif ind == 1 : return True ,None
         elif ind == 2 : return None ,True
         else          : return None ,False
      elif Ones == 2  :
         ind1 = self.val.index(0)
         ind2 = self.val.index(0,ind1+1)
         if   ind1 == 0 and ind2 == 1 : return 'Null',None
         elif ind1 == 2 and ind2 == 3 : return  None ,'Null'
         elif ind1 == 0 and ind2 == 3 : return  False,False
         else                         : return  True ,True
      elif Ones == 1  :  
         ind = self.val.index(1)
         if   ind == 0 : return True ,'Null'
         elif ind == 1 : return False,'Null'
         elif ind == 2 : return 'Null',False
...
class ConstraintTable(dict):
   ...
   def getOutSurfaces(self):
      out = [] 
      for k in self.keys():
         if self[k][k].val != 0 :  out.append(k)
      return out       
   ...
   def getConstraintSet(self,valname):
       ...
       for k in self.keys():
          ...
          if FValue == 'Null' :
               FNull = True
               FValue = None
          if TValue is not None  : trueSet[k]  = TValue
          if FValue is not None  : falseSet[k] = FValue

       if TNull : trueSet  = None
       if FNull : falseSet = None
...
def combineDiagElements(d1,d2):
    ...
    if   d1.val == 0 and d2.val == 0 : return CTelement((1,1,1,1))
    elif d1.val == 1 and d2.val == 0 : return CTelement((1,1,0,0))
    elif d1.val ==-1 and d2.val == 0 : return CTelement((0,0,1,1))
    elif d1.val == 0 and d2.val == 1 : return CTelement((1,0,0,1))
    elif d1.val == 0 and d2.val ==-1 : return CTelement((0,1,1,0))
    elif d1.val == 1 and d2.val == 1 : return CTelement((1,0,0,0))
    elif d1.val == 1 and d2.val ==-1 : return CTelement((0,1,0,0))
    elif d1.val ==-1 and d2.val == 1 : return CTelement((0,0,0,1))
    elif d1.val ==-1 and d2.val ==-1 : return CTelement((0,0,1,0))
...
def buildCTableFromSolids(Box,SurfInfo,option='diag'):
     ...
     if type(surfaces[surfaceList[0]]) is GEOUNED_Surface :
        ...
        for s in surfaceList: surfaces[s].buildShape(Box.BoundBox)
     ...
     for i,s1 in enumerate(surfaceList):
         ...
         if option == 'diag' :continue
         if splitRegions is None : continue  # loop, no region to be split by s2
         for j,s2 in enumerate(surfaceList[i+1:]) :
            ...
            for solid in posS1 :
                ...
                if pos == (1,1) : break   # s2 intersect S1 Region
                if pos0 is None :
                   pos0 = pos
                else:
            ...
            for solid in negS1 :
                ...
                if neg == (1,1) : break   # s2 intersect S1 Region
                if neg0 is None :
                   neg0 = neg
                else:
     ...
     if option != 'diag' : CTable.fillMissingElements()
     return CTable
         ...
         #    return ((0,0),(0,1)),None
          ...
          for s in comsolid.Solids:
             ...
             if sgn == 1:
                 ...
                 negSol.append(s)
          return 0, (posSol,negSol) # return the diagonal element of the Constraint Table for s1, and solids to be split by s2       
