...
class Surfaces_dict(dict):
   ...
   def addCylinder(self,cyl,fuzzy=False):
        ...
        for i,c in enumerate(self['Cyl']):
          if  BF.isSameCylinder(cyl.Surf,c.Surf,dtol=tol.cyl_distance,atol=tol.cyl_angle,relTol=tol.relativeTol,fuzzy=(fuzzy,c.Index)):
                addCyl = False
                index = c.Index
   ...
   def addCone(self,cone):
        ...
        for i,c in enumerate(self['Cone']):
          if  BF.isSameCone(cone.Surf,c.Surf,dtol=tol.kne_distance,atol=tol.kne_angle,relTol=tol.relativeTol):
                addCone = False
                index = c.Index
   ...
   def addSphere(self,sph):
        ...
        for i,s in enumerate(self['Sph']):
          if  BF.isSameSphere(sph.Surf,s.Surf,tol.sph_distance,relTol=tol.relativeTol):
                addSphere = False
                index = s.Index
   ...
   def addTorus(self,tor):
        ...
        for i,s in enumerate(self['Tor']):
          if  BF.isSameTorus(tor.Surf,s.Surf,dtol=tol.tor_distance,atol=tol.tor_angle,relTol=tol.relativeTol):
                addTorus = False
                index = s.Index
