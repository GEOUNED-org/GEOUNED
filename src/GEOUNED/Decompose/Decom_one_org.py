...
def cutFullCylinder(solid):
    ...
    for pp in Planes :
       ...
       comsolid=BOPTools.SplitAPI.slice(solid,tools,'Split', tolerance = 0.0)
    ...
    comsolid=BOPTools.SplitAPI.slice(solid,tool,'Split', tolerance = 0.0)
...
def TorusBoundPlanes(face,boundBox):
    ...
    planes = []
    if isSameValue(params[1]-params[0],2.0*math.pi,tol.value) : return planes
...
def ExtractSurfaces(solid,kind,UniverseBox,MakeObj=False,realPlane=False):
    if kind == 'All':
       fuzzy = True 
       solidParts = []
    ...
    for solid_GU in solidParts:
       ...
       for face in solid_GU.Faces:
         ...
         if surf == '<Plane object>' and kind in ['Planes','All'] :
           ...
           if kind == 'Planes' and not realPlane:    
             #add planes if cylinder axis is cut by a plane (plane quasi perpedicular to axis)
             for p in CylBoundPlanes(face,UniverseBox) :
                 if MakeObj : p.buildSurface()
                 Surfaces.addPlane(p)

             # add plane if cylinder face is not close and no-convex
             for p in Plane2ndOrder(solid_GU,face,flag_inv,False) :
                plane = UF.GEOUNED_Surface(('Plane',(p.Surface.Position,p.Surface.Axis,rad,dimL)),UniverseBox)
                if MakeObj : plane.buildSurface()
                Surfaces.addPlane(plane)
           ...
           if kind == 'Planes' and not realPlane:
             ...
             for p in Plane2ndOrder(solid_GU,face,flag_inv,False) :
                plane = UF.GEOUNED_Surface(('Plane',(p.Surface.Position,p.Surface.Axis,dimR,dimL)),UniverseBox)  
                if MakeObj : plane.buildSurface()
                Surfaces.addPlane(plane)
...
def GenPlaneCylinder(face,solid):
    ...
    Faces_p=[]
    for elem in face_index_0:
        Faces_p.append(solid.Faces[elem])
    ...
    for ind in face_index[1:]:
        if solid.Faces[ind].Area<=1e-3:
            face_index.remove(elem)
...
def SplitPlanes_new(Solids,UniverseBox):
    ...
    while True: 
      newBases = []
      for base in Bases:
         cutSolids = splitPPlanes_new(base,UniverseBox,True)
         if len(cutSolids) == 1:
    ...
    while True: 
      newBases = []
      for base in Bases:
         cutSolids = splitPPlanes_new(base,UniverseBox,False)
         if len(cutSolids) == 1:
           simpleSolid.extend(cutSolids)
         else:
           newBases.extend(cutSolids)
    ...
    for iplane in range(3) :
       ...
       for item in Bases :
         ...
         if len(Planes[imin]) != 0:
            ...
            comsolid=BOPTools.SplitAPI.slice(base,Tools,'Split', tolerance = 0)
            if len(comsolid.Solids) == 1 :
...
class parallelPlanes:
    ...
    def append(self,plane):
       if isParallel(plane.Surf.Axis,self.Axis,tol.angle):
...
def splitPPlanes_new(solid,UniverseBox,realPlane):
    SPlanes = ExtractSurfaces(solid,'Planes',UniverseBox,realPlane=realPlane)
...
def splitPPlanes_org(solid,UniverseBox):
    ...
    for p in SPlanes['P'] :
       p.buildSurface()
       comsolid=BOPTools.SplitAPI.slice(solid,[p.shape],'Split', tolerance = 0.0)
       if len(comsolid.Solids) > 1 :
         outSolid = comsolid.Solids
         break
    return outSolid   
        ...
        for solid in Base :
           ...
           if len(Surfaces[kind]) == 0 :
              ...
              for s in Surfaces[kind]:
                 s.buildSurface()
                 try :
                    comsolid=BOPTools.SplitAPI.slice(solid,[s.shape],'Split', tolerance = 0)
...
def split2ndOPlane(solid):
    ...
    for p in planes:
      comsolid=BOPTools.SplitAPI.slice(solid,[p],'Split', tolerance = 0.0)
